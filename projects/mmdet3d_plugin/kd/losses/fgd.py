...
class FeatureLoss(nn.Module):
    ...
    def forward(self, preds_S, preds_T, gt_bboxes, img_metas):
        ...
        wmin,wmax,hmin,hmax = [],[],[],[]
        # TODO : check the gt_bboxes size: N*6*1*4, batch_size = N, camera_num = 6, queue=1, bbox=4
        for i in range(N):
            if len(gt_bboxes[i//Cams][0][i%Cams]) == 0:
                new_boxxes = torch.ones((1,4)).to(gt_bboxes[i//Cams][0][i%Cams].device)
                new_boxxes[:, 0] = 0
                new_boxxes[:, 2] = 0
                new_boxxes[:, 1] = 0
                new_boxxes[:, 3] = 0
            else:
                new_boxxes = torch.ones_like(gt_bboxes[i//Cams][0][i%Cams])
                new_boxxes[:, 0] = gt_bboxes[i//Cams][0][i%Cams][:, 0]/img_metas[i//Cams][0]['img_shape'][i%Cams][1]*W
                new_boxxes[:, 2] = gt_bboxes[i//Cams][0][i%Cams][:, 2]/img_metas[i//Cams][0]['img_shape'][i%Cams][1]*W
                new_boxxes[:, 1] = gt_bboxes[i//Cams][0][i%Cams][:, 1]/img_metas[i//Cams][0]['img_shape'][i%Cams][0]*H
                new_boxxes[:, 3] = gt_bboxes[i//Cams][0][i%Cams][:, 3]/img_metas[i//Cams][0]['img_shape'][i%Cams][0]*H
            ...
            for j in range(len(gt_bboxes[i//Cams][0][i%Cams])):
    ...
    def get_mask_loss(self, C_s, C_t, S_s, S_t):
        ...
        mask_loss = torch.mean(torch.abs((C_s-C_t)))/len(C_s) + torch.mean(torch.abs((S_s-S_t)))/len(S_s)
